...
app.post('/book', async (req, res) => {
    ...
    try {
        // --- NEAREST STORE LOGIC ---
        let assignedStoreId = 1; // Default: Bhimavaram Central
        ...
        if (lat && lng && stores.length > 0) {
            ...
            stores.forEach(store => {
                const dist = Math.sqrt(Math.pow(store.lat - lat, 2) + Math.pow(store.lng - lng, 2)); // Simplistic distance
                if (dist < minDistance) {
                    minDistance = dist;
        ...
        res.json({ 
            ...
            message: `Success! Booking ID #${bookingId}. Assigned to ${storeName}.`
        });
    } catch (err) {
...
// --- ADVANCED DELIVERY BOY APP API ENDPOINTS ---

// 1. Get Pending Requests (Broadcast to all Delivery Boys)
app.get('/api/delivery-requests', async (req, res) => {
    try {
        const result = await pool.query("SELECT * FROM rentals WHERE status = 'pending' ORDER BY created_at ASC");
        res.json(result.rows);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
// 2. Accept Delivery (First boy to click ACCEPT wins)
app.post('/api/accept-delivery/:id', async (req, res) => {
    const { id } = req.params;
    const { partnerId, partnerName, partnerPhone } = req.body;
    try {
        // Use a transaction or simple atomic update if no race conditions, but PG UPDATE with WHERE is atomic
        const result = await pool.query(
            "UPDATE rentals SET status = 'accepted', delivery_boy_name = $1, delivery_boy_phone = $2, track_status = 'assigned' WHERE id = $3 AND status = 'pending' RETURNING *",
        ...
        if (result.rows.length > 0) {
            ...
            // SEND SMS TO USER ABOUT ASSIGNED BOY
            const userMsg = `✅ Your RentPlay order "${order.item}" is confirmed! Assigned Delivery Boy: ${partnerName} (${partnerPhone}). Live tracking enabled.`;
            await sendSmsAlert(userMsg); // You could send to the specific user phone if using real Twilio APIs rather than owner fallback
            // (Note: To send directly to user, you'd use client.messages.create to order.phone, assuming valid number)
            
            res.json({ success: true });
        } else {
            // It means another delivery boy already took it, or it doesn't exist
            res.json({ success: false, message: "Order already accepted by another partner!" });
        }
    } catch (err) {
        res.status(500).json({ success: false, error: err.message });
    }
...
app.get('/api/my-trips', async (req, res) => {
    const { partnerId } = req.query; // in UI we used ID
    const { partnerName } = req.query; // actually we stored name/phone in rentals table, let's just match by Name or Phone
    
    // In our quick schema addition, we didn't add delivery_boy_id, only name and phone. Let's return anything not returned
    try {
        const result = await pool.query(
